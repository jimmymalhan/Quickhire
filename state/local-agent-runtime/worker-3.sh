...
LOG="$STATE/worker-3.log"
echo $$ > "$STATE/worker-3.pid"
log(){ printf '[%s] [WORKER-3] %s\n' "$(date +%H:%M:%S)" "$1" | tee -a "$LOG"; }
...
try:
  ...
  for t in tasks:
    if t['id']==4: t['status']='$1'; t['worker']='worker-3'
  json.dump(tasks,open('$STATE/backlog.json','w'),indent=2)
except: pass
" 2>/dev/null||true
}

log "=== WORKER-3 started — task[4]: Feat:_rate_limiting_+_session_management ==="
...
# Create branch
br="feat/rate-limiting-$(date +%s)"
git branch -D "$br" 2>/dev/null||true
if ! git checkout -b "$br" 2>/dev/null; then
  log "branch conflict — retrying"
  learn "branch conflict on feat/rate-limiting — use timestamp suffix"
  br="feat/rate-limiting-w3-$(date +%s)"
  git checkout -b "$br" 2>/dev/null||{ log "ABORT branch failed"; update_task_status "failed"; exit 1; }
fi

log "Working on: Feat:_rate_limiting_+_session_management"
...
case "4" in
  1) # Untrack AI rules
    ...
    if ! git diff --cached --quiet 2>/dev/null; then
      git commit -m "fix: untrack AI rules from git [worker-3]
...
AI rule files (.claude/, CLAUDE.md) are local-only.
  ...
  *) # Placeholder — write stub file for feature
    ...
    touch "$ROOT/src/automation/.worker-3-task4" 2>/dev/null||true
    log "Stub created for task 4 — node_modules needed for full impl" ;;
esac

update_task_status "done"
learn "task 4 completed by worker-3 — Feat:_rate_limiting_+_session_management"
log "=== WORKER-3 DONE ==="

# Worker stays alive monitoring
while true; do
  printf '[%s] [WORKER-3] idle — monitoring\n' "$(date +%H:%M:%S)" >> "$LOG"
  sleep 120; done
